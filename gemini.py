...
class gemini:
    ...
    def ticker_data(self, currency: str) -> dict:
        response = requests.get(f"https://api.gemini.com/v2/ticker/{currency}")
        response.raise_for_status()
        return response.json()

    def analyze_data(self, data: dict) -> dict:

        prices = float_array = np.asarray(data['changes'], dtype=np.float64)
        ...
        # Calculate absolute value of deviation as notional value (assuming deviation itself as notional value)
        price_change_value = np.sum(np.abs(deviations))

        self.log_debug(f"price change value: {price_change_value}")

        return {"std_deviation": std_deviation, "price_change_value": price_change_value}
    ...
    def log_currency(self, data, deviation_data):
        ...
        if deviation_data["std_deviation"] >= self.deviation:
            ...
            data_block = {"last_price": data["close"], "average": "", "sdev": deviation_data["std_deviation"], "change": deviation_data["price_change_value"]}
...
if __name__ == "__main__":
    ...
    parser.add_argument('-c', '--currency', default="ALL", help="Currency trading currency or ALL (default: ALL)")
    parser.add_argument('-d', '--deviation', type=float, default="1", help="Standard deviation threshold (default: 1.0)")
    parser.add_argument('-l', '--log_level', default="INFO", help="Log level of DEBUG, INFO, or ERROR (default: INFO)")

    args = parser.parse_args()

    gemini_currency_report = gemini(currency=args.currency, deviation=args.deviation, log_level=args.log_level)
